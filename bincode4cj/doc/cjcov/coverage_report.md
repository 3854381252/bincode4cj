...
- **总体覆盖率**：83.6% → 82.7%（小幅下降）
- **decode.cj**：52.4% → 58.7%（新增 9-16 元组解码代码，部分有测试覆盖但未完全覆盖；文件从 663 行增长到 1088 行）
- **encode.cj**：86.8% → 87.7%（小幅提升）
- **覆盖率下降原因**：新增 9-16 元组编解码代码，其中 10-16 元组的深层嵌套 match 分支未被测试完全覆盖，拉低了总体覆盖率。
